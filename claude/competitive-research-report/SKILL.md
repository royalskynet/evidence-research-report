...
執行任何調研任務前，先 Read `profile.local.md`，從中取得：委託方身分（調用者與所屬組織）、報告署名規則、內部考核維度、競品對標名單，以及本地反偵測渲染通道指令。

若 `profile.local.md` 不存在，則依 `profile.example.md` 所列欄位，向調用者逐項確認後再開始調研。
...
3. **Cross-model 抽查（若 `ARS_CROSS_MODEL` 已設定）**：

```bash
# 檢查是否有 Cross-model 可用
source ~/.zshrc 2>/dev/null
if [ -n "$ARS_CROSS_MODEL" ] && [ -n "$GOOGLE_AI_API_KEY" ]; then
  echo "CROSS_MODEL_AVAILABLE: $ARS_CROSS_MODEL"
else
  echo "CROSS_MODEL_AVAILABLE: none"
fi
```

若可用，從文末清單隨機抽 5 條，構造以下 prompt 送 Gemini 驗證：

```bash
PROMPT="以下是一組來自風控調研報告的主張與來源配對，請逐條判斷：主張內容是否與來源 URL 的頁面相符？回答格式：MATCH / MISMATCH（附說明）/ CANNOT_VERIFY（頁面不可達）
...
curl -s "https://generativelanguage.googleapis.com/v1beta/models/${ARS_CROSS_MODEL}:generateContent?key=$GOOGLE_AI_API_KEY" \
  ...
  -d "{\"contents\":[{\"parts\":[{\"text\":$(echo "$PROMPT" | jq -Rs .)}]}],\"generationConfig\":{\"temperature\":0.1,\"maxOutputTokens\":1000}}" \
  | jq -r '.candidates[0].content.parts[0].text // .error.message'
```

任何 `MISMATCH` → 回頭核查該主張，修正或刪除後才能定稿。
...
- **署名**：依 profile 指定，未指定則不主動加註

---

<!-- 2026-08-28: 追加「報告文本純學術性」鐵律，因反欺詐調研輸出曾殘留 fetch／agent／重試等工作過程語彙 -->
...
- **代理與模型調度**：agent、subagent、delegate、Explore、general-purpose、cross-model 驗證、跨模型、agent reach
- **迭代與修復過程**：重試、二次檢索、retry、補齊清單、韌性、rollback、修正後、經抓取後
- **輪次與流程分段用語**：第一輪、第二輪、two-round、round 1、round 2、補強、定向補強、缺口清單、資料缺口清單（現有「代理與模型調度」類已涵蓋 agent／subagent／Camoufox，此處是額外堵住「輪次」相關字眼，避免報告透露分階段作業痕跡）
- **英文夾雜的臨時語**：vendor、marketing、workaround、fallback（技術術語照原文專有名詞除外，如 SDN、KYC、AML、GDPR、VARA、AIT、E.164、ITU-T E.371、carrier_risk_category 等命名信號欄位）
...
- 內部才有的數字（正樣本率、複核工時、內部基線）改在配套 PRD 的「內部提供輸入項」處理，不出現在調研正文的資料缺口章節。
...
分析委託方與同業主流虛擬資產服務商（VASP）之策略差異，具體對標名單依 profile 提供代入。若委託方提供內部考核維度作為方案評估依據，制定建議時應納入呼應，但考核維度本身不得出現在對外交付的報告文本中。
...
- **分類 ❶（本地反偵測渲染通道）**：同一批要抓的 URL 合併成一次批次抓取，依 profile 指定的本地反偵測渲染通道，介面為 `<URL 或 @urls.txt>`，輸出 `URL / STATUS / TITLE / BODY`，不逐條重試。抓到 BODY 有實質內容 → 依鐵律 1／鐵律 2 正常編號引用；STATUS 非 200 或 BODY 空 → 該項目判定放棄，維持原本鐵律 3 標記。
- **分類 ❷（agent reach 通道）**：派遣 `Agent` 工具（`subagent_type: general-purpose`），同一批同類缺口項目合併成一次派遣，要求子代理換關鍵字角度／換語言／找替代原始來源後回報「找到的具體事實＋來源 URL」或「確認查無可信來源」。子代理回報找不到 → 該項目判定放棄，維持原本鐵律 3 標記。
...
【第一輪】階段 4：盤點缺口工作清單（無缺口則跳過第二輪）
  ↓
【第二輪】依失敗原因分類，camoufox／agent reach 各自單一嘗試，失敗即放棄
  ↓
【整合定稿】合併兩輪結果，缺口處置寫回原段落，差距分析 + 建議
  ↓
鐵律 4：定稿前引用完整性抽查
  ↓
Cross-model 驗證（若 ARS_CROSS_MODEL 可用）
  ↓
附完整 §七 參考來源清單
  ↓
告知調用者：本次調研任務結束
...
- **遇到矛盾資料**：保留兩個說法並標注出處，讓調用者判斷
- **官方頁面找不到**：優先嘗試 Wayback Machine 或業界媒體（CoinDesk、The Block、律動）
- **每個搜尋關鍵字試 2–3 個變體**（英文 + 中文）以提高覆蓋率
- **表格中的數字**：直接附 `[N]`，不要把引用只放在下方文字說明中
- **ARS_CROSS_MODEL 可用時**：優先抽查爭議性或難以獨立核實的主張，而非隨機抽查
- **第二輪通道選擇**：camoufox 通道只用於官方頁面確實存在但打不開（防爬／JS 渲染）的情況；根本找不到任何來源時該走 agent reach 換角度找，不要對同一個打不開的 URL 反覆嘗試
